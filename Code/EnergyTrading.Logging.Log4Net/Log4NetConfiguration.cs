namespace EnergyTrading.Logging.Log4Net
{
    ...
    public class Log4NetConfiguration : IConfigurationTask
    {
        public Log4NetConfiguration()
        {
            try
            {
                ...
                if (section != null)
                {
                    ...
                    this.LoggingFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, configSource);
                }
                else
                {
                    this.LoggingFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "logger.config");
                }
            }
            catch (Exception)
            {
        ...
        {
            if (File.Exists(this.LoggingFile))
            {
                var fi = new FileInfo(this.LoggingFile);
