namespace EnergyTrading.Logging.Log4Net
{
    ...
    public class TimerFlushAppender : AppenderSkeleton, IAppenderAttachable
    {
        ...
        {
            if ((this.StartTime == default(DateTime)) || (this.EndTime == default(DateTime)))
            {
                return true;
            }

            var now = DateTime.UtcNow;
            if (this.EndTime.TimeOfDay < this.StartTime.TimeOfDay)
            {
                return (now.TimeOfDay >= this.StartTime.TimeOfDay) || (now.TimeOfDay < this.EndTime.TimeOfDay);
            }
        ...
        {
            if (this.appenderAttachedImpl != null && this.IsInTimeWindow())
            {
