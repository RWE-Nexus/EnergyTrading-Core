namespace EnergyTrading.FileProcessing.FileProcessors
{
    ...
    /// </para>
    /// </summary>
    public class EventBasedFileProcessor : IFileProcessor
    {
        ...
        private IFileFilter AdditionalFilter { get; set; }
        ...
        protected void TimerElapsed(object sender, ElapsedEventArgs args)
        {
            ...
            try
            {
                ...
                foreach (var s in Directory.EnumerateFiles(this.DropPath, "*.*", option).Where(x => File.GetLastAccessTimeUtc(x) < (DateTime.UtcNow - this.Endpoint.RecoveryInterval)))
                {
                    ...
                    try
                    {
                        ...
                        if ((s.EndsWith(fileDropExtension, StringComparison.InvariantCultureIgnoreCase) || (fileDropExtension == ".*" && !s.EndsWith(InProgress, StringComparison.InvariantCultureIgnoreCase)))
                            && this.AdditionalFilter.IncludeFile(s))
                        {
                            Logger.Debug("Processing file " + source);
                            var inprog = Path.ChangeExtension(s, "." + this.InstanceId + "enqueue" + InProgress);
                ...
                if (this.sufferedNetworkError)
                {
                    this.StopFileWatcher();
                    this.StartFileWatcher();
                    this.sufferedNetworkError = false;
                }
            }
        ...
        {
            var successDir = this.SuccessPath;
            var dirpath = this.GetPathWithoutWatchedFolder(originalFilePath);
            if (this.SuccessPath.Contains("%filepath%"))
            {
                successDir = this.SuccessPath.Replace("%filepath%", Path.GetDirectoryName(originalFilePath));
        ...
        {
            var failDir = this.FailurePath;
            var dirpath = this.GetPathWithoutWatchedFolder(originalFilePath);
            if (this.FailurePath.Contains("%filepath%"))
            {
                failDir = this.FailurePath.Replace("%filepath%", Path.GetDirectoryName(originalFilePath));
                dirpath = Path.GetFileName(originalFilePath);
            }
        ...
        {
            return path.Substring(this.DropPath.EndsWith(Path.DirectorySeparatorChar.ToString(CultureInfo.InvariantCulture)) ? this.DropPath.Length : this.DropPath.Length + 1);
        }
        ...
        private void NewFileNotification(object sender, FileSystemEventArgs e)
        {
            try
            {
                ...
                this.pendingFiles.Enqueue(e.FullPath);
