namespace EnergyTrading.Mapping
{
        ...
        where TSource : class, new()
    {
        ...
        public XElement Map(TSource source, string nodeName, string xmlNamespace = null, bool outputDefault = false)
        {
            ...
            {
                if (!string.IsNullOrEmpty(this.XmlType))
                {
                    ...
                    destination.Add(
                        new XAttribute(
                        this.QualifiedName("type", XsiNamespace),
                        this.QualifiedPrefixName(this.XmlType, this.XmlTypeNamespacePrefix)));
                }
            }
        ...
        {
            return new XElement(this.QualifiedName(this.NodeName, this.Namespace));
        }

        protected List<XElement> ElementList(IEnumerable<TSource> source, string nodeName, string nodeNamespace = "")
        {
            return source.Select(c => this.Map(c, nodeName, string.IsNullOrWhiteSpace(nodeNamespace) ? this.Namespace : nodeNamespace)).ToList();
        }
        ...
        protected XElement XElement(string name, DateTime value, string xmlNamespace = null, bool outputDefault = false, string format = null)
        {
            ...
            return this.XElement(name, formattedValue, xmlNamespace, outputDefault);
        }
