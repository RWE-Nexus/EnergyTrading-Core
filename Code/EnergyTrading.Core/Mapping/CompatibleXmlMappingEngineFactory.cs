namespace EnergyTrading.Mapping
{
    ...
    public class CompatibleXmlMappingEngineFactory : IXmlMappingEngineFactory
    {
        ...
        public IXmlMappingEngine Find(string version)
        {
            try
            {
                var engine = this.ObtainExactVersion(version) ?? this.ObtainLowerMinorVersion(version);
                if (engine != null)
                {
                    return engine;
                }
            ...
            {
                throw this.ThrowXmlEngineResolutionError(version, ex);
            }

            throw this.ThrowXmlEngineResolutionError(version);
        }
        ...
        private Exception ThrowXmlEngineResolutionError(string asmVersion, Exception exception = null)
        {
            ...
            if (!this.registry.SchemaExists(schemaVersion.Schema))
            {
            ...
            else 
            {
                ...
                if (this.ObtainExactVersion(new Version(schemaVersion.Version.Major - 1, 0).ToAsmVersion(schemaVersion.Schema)) != null)
                {
