namespace EnergyTrading.Test.TeamCity
{
    public class TeamCityBuildLogger : ITeamCityBuildLogger
    {
        ...
        {
            var text = includeStandardTeamCityStatusText ? string.Format("{{build.status.text}} {0}", this.StripInvalidMarkupCharacters(message)) : this.StripInvalidMarkupCharacters(message);
        ...
        {
            System.Console.Out.WriteLine("##teamcity[buildStatus status='SUCCESS'{0}]", message == null ? string.Empty : string.Format(" text='{0}'", this.StripInvalidMarkupCharacters(message)));
        }
        ...
        {
            System.Console.WriteLine("##teamcity[{0} name='{1}' out='{2}']", isError ? "testStdErr" : "testStdOut", testName, this.StripInvalidMarkupCharacters(message));
        }

        public void LogTestFailed(string testName, string message)
        {
            System.Console.WriteLine("##teamcity[testFailed name='{0}' message='{1}']", testName, this.StripInvalidMarkupCharacters(message));
        }
        ...
        {
            this.LogTestStarted(testName);
            if (testLogMessages != null)
            {
                foreach (var message in testLogMessages)
                {
                    this.LogTestOutput(testName, message);
                }
            }
            if (testErrorMessages != null)
            {
                ...
                {
                    this.LogTestOutput(testName, message, true);
                }
            }
            if (failureMessage != null)
            {
                this.LogTestFailed(testName, failureMessage);
            }
            this.LogTestFinished(testName, duration);
        }

        public void LogBuildProgressMessage(string message)
        {
            System.Console.WriteLine("##teamcity[progressMessage '{0}']", this.StripInvalidMarkupCharacters(message));
        }

        public void LogBuildProgressStartMessage(string message)
        {
            System.Console.WriteLine("##teamcity[progressStart '{0}']", this.StripInvalidMarkupCharacters(message));
        }

        public void LogBuildProgressFinishMessage(string message)
        {
            System.Console.WriteLine("##teamcity[progressFinish '{0}']", this.StripInvalidMarkupCharacters(message));
        }
