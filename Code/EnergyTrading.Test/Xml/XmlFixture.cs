namespace EnergyTrading.Test.Xml
{
    ...
    public abstract class XmlFixture : Fixture
    {
        ...
        {
            var result = expectedXml.CheckXml(candidateXml, fullComparison, this.DiffFilePath);
        ...
        {
            var result = expectedNode.CheckXml(candidateXml, fullComparison, this.DiffFilePath);
            if (!string.IsNullOrEmpty(result))
            {
                Assert.Fail(result);
            }
        ...
        {
            var result = expected.CheckXml(candidate, fullComparison, this.DiffFilePath);
            if (!string.IsNullOrEmpty(result))
            {
                Assert.Fail(result);
            }
        ...
        public void VerifyXmlFile(string path)
        {
            ...
            config.ValidationEventHandler += this.ValidationCallBack;
